...
void CLOCK_SetClkConfig(clock_config_t *psConfig)
{
    ...
    {
        OCCS->OSCTL2 &= ~((uint16_t)OCCS_OSCTL2_ROPD200K_MASK);
    }
    ...
    if (!(psConfig->bFircEnable))
    {
        SIM->MISC0 &= ~((uint16_t)SIM_MISC0_IRC48M_EN_MASK);
    }
...
void OCCS_DriverIRQHandler(void)
{
    ...
    if (OCCS_CTRL_PRECS(kCLOCK_MstrOscClkSrcExt) == (OCCS->CTRL & OCCS_CTRL_PRECS_MASK))
    {
        ...
        OCCS->CTRL = (OCCS->CTRL & (~((uint16_t)OCCS_CTRL_PRECS_MASK))) | OCCS_CTRL_PRECS(kCLOCK_MstrOscClkSrcFircDiv6);
        __NOP();
        __NOP();
        __NOP();
        __NOP();
