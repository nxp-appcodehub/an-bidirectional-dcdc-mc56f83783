...
#define FSL_SIM_DRIVER_VERSION (MAKE_VERSION(2, 2, 0))
/*@}*/
 ...
 * @ingroup sim
 * @{
 * The current SIM driver version is 2.2.0.
 *
 * - 2.2.0
...
static inline uint16_t SIM_GetSoftwareControlData(SIM_Type *base, sim_software_contrl_register_index_t eIndex)
{
    ...
    {
        return (*((volatile uint16_t *)&(base->SCR4) + (uint16_t)eIndex - (uint16_t)kSIM_SCR4));
    }
}
                                              ...
                                              uint16_t u16Value)
{
    ...
    {
        (*((volatile uint16_t *)&(base->SCR4) + (uint16_t)eIndex - (uint16_t)kSIM_SCR4)) = u16Value;
    }
}
