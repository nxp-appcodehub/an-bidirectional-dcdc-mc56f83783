...
#ifndef _FSL_COMMON_DSC_H_
#define _FSL_COMMON_DSC_H_
...
#define USEC_TO_COUNT(us, clockFreqInHz) (uint32_t)(float)(((float)(clockFreqInHz) / (float)1000000.0f) * (float)(us))
/*! Macro to convert a raw count value to microsecond */
#define COUNT_TO_USEC(count, clockFreqInHz) \
    (uint32_t)(float)(((float)count / (float)(clockFreqInHz)) * (float)1000000.0f)

/*! Macro to convert a millisecond period to raw count value */
#define MSEC_TO_COUNT(ms, clockFreqInHz) (uint32_t)(float)(((float)(clockFreqInHz) / (float)1000.0f) * (float)(ms))
/*! Macro to convert a raw count value to millisecond */
#define COUNT_TO_MSEC(count, clockFreqInHz) \
    (uint32_t)(float)(((float)(count) / (float)(clockFreqInHz)) * (float)1000.0f)
/* @} */
